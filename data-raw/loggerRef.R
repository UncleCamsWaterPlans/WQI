...
URL1_Wrksp<-paste("https://api.eagle.io/api/v1/nodes?attr=_id,name&filter=_class($eq:io.eagle.models.node.Workspace)",sep="")
API1_Wrksp <- GET(URL1_Wrksp,
              add_headers('X-Api-Key' = APIKEY,
                          'Content-Type' = "application/json"))
Wrksp_content=fromJSON(rawToChar(API1_Wrksp$content))
...
for (i in 1:(dim(Wrksp_content)[1])) {
  ...
  URL2_Node <- paste("https://api.eagle.io/api/v1/nodes/?attr=_id,name&filter=parentId($eq:",WSID,")",sep="")
  API2_Node <- GET(URL2_Node,
                    add_headers('X-Api-Key' = APIKEY,
                                'Content-Type' = "application/json"))
  Node_content=fromJSON(rawToChar(API2_Node$content))
...
for (i in 1:(dim(loggerRef)[1])) {
  ...
  URL3_RTD <- paste("https://api.eagle.io/api/v1/nodes/?attr=_id,name&filter=parentId($eq:",SiteID,")",sep="")
  API3_RTD <- GET(URL3_RTD,
                   add_headers('X-Api-Key' = APIKEY,
                               'Content-Type' = "application/json"))
  RTD_content=fromJSON(rawToChar(API3_RTD$content))
  RTD <- RTD_content %>%
    filter(name == "Real-time Data")
  RTD<-RTD[1,1]

  URL4_Logger <- paste("https://api.eagle.io/api/v1/nodes/?attr=_id,name&filter=parentId($eq:",RTD,")",sep="")
  API4_Logger <- GET(URL4_Logger,
                     add_headers('X-Api-Key' = APIKEY,
                                 'Content-Type' = "application/json"))
  Logger_content=fromJSON(rawToChar(API4_Logger$content))
...
for (i in 1:(dim(loggerRef)[1])) {
  ...
  URL5_param <- paste("https://api.eagle.io/api/v1/nodes/?attr=_id,name&filter=parentId($eq:",LoggerID,")",sep="")
  API5_param <- GET(URL5_param,
                  add_headers('X-Api-Key' = APIKEY,
                              'Content-Type' = "application/json"))
  param_content=fromJSON(rawToChar(API5_param$content))
  #param <- param_content %>%
